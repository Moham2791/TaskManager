...
public class Utils {
    ...
    public String checkName(String name){
       ...
       try {
           if (!(name.length() > 255) && !conainsNumber(name)) {
    ...
    public boolean conainsNumber(String input) {
        if (input == null) {
            return false;
        }
    ...
    public String checkTaskType(String taskType) {
        try {
            if (taskType != null && !taskType.isEmpty() && taskType.length() <= 50 && !conainsNumber(taskType)) {
                return taskType;
            }
